...
export default function ScreeningPage() {
  ...
  const visible = isReferral
    ? rows.filter((r) => r.referralName === user?.name)
    : rows;
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (isReferral && !form.id) data.referralName = user?.name;
    if (form.id) {
      persist(rows.map((r) => r.id === form.id ? data : r));
      toast.success("Data screening diperbarui");
    } else {
      persist([...rows, { ...data, id: uid("sc_"), createdAt: new Date().toISOString() }]);
      toast.success("Data screening ditambahkan");
    }
    setOpen(false); setForm(empty);
  };

  const edit = (r: any) => { setForm(r); setOpen(true); };
  const del = (id: string) => { if (confirm("Hapus data ini?")) persist(rows.filter((r) => r.id !== id)); };

  return (
    <div>
      <PageHeader
        title="Data Screening Affiliate"
        subtitle="Database mentah affiliate sebelum diverifikasi (Pending / Diterima / Ditolak)"
        actions={
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setForm(empty); }}>
            <DialogTrigger asChild>
              <Button className="bg-brand-blue hover:bg-brand-blue-dark text-white"><Plus className="h-4 w-4 mr-1" />Tambah Screening</Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl">
              <DialogHeader><DialogTitle>{form.id ? "Edit" : "Tambah"} Screening Affiliate</DialogTitle></DialogHeader>
              <form onSubmit={submit} className="grid grid-cols-2 gap-3">
                ...
                <div><Label className="text-xs">No HP Affiliate</Label><Input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></div>
                ...
                <div>
                  <Label className="text-xs">Active / Non Aktif</Label>
      ...
      <DataTable
        ...
        cols={[
          ...
          { key: "active", label: "Aktif", render: (r) => <StatusBadge status={r.active} /> },
          {
            key: "act", label: "Aksi", render: (r) => (
              <div className="flex gap-1">
                ...
                {!isReferral && <Button size="icon" variant="ghost" className="h-7 w-7 text-brand-red hover:bg-brand-red-soft" onClick={() => del(r.id)}><Trash2 className="h-3.5 w-3.5" /></Button>}
              </div>
            ),
          },
        ]}
      ...
      <Dialog open={!!view} onOpenChange={(o) => !o && setView(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Detail Screening</DialogTitle></DialogHeader>
          {view && (
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                ["Status", view.status], ["Aktif", view.active],
              ].map(([k, v]) => (
